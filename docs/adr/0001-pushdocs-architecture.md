...
Документ определяет архитектуру первой полноценной версии. Статус `proposed` означает предложение к разработке, а не наличие реализации. Название PushDocs взято из текущего репозитория.
...
packages/
  ...
  git/                 Работа с Git-объектами, коммитами и слияниями
  providers/           Адаптеры GitHub и GitLab
  content/             Docusaurus, Markdown/MDX и конфигурация компонентов
  editor/              Визуальный редактор, исходник и сравнение версий
  api-client/          Единый браузерный транспорт и ключи кеша
  ui/                  Base UI, стили и общие элементы интерфейса
deploy/                Dockerfile, Compose, Caddy и bootstrap
...
Server Components используются для оболочки и первоначальных данных. Редактор и обсуждения являются клиентскими компонентами. Изменяемые приватные данные читаются без общего серверного кеша; клиентский кеш разделяется по пользователю, проекту, ветке и документу. При выходе или смене пользователя приватный кеш очищается. Все браузерные HTTP-запросы проходят через `api-client`.

Основной API оформляется через Route Handlers `/api/v1`. Server Actions допустимы для локальных форм оболочки, если вызывают те же сценарии `core`. Git и длительные интеграции выполняются worker после записи задачи в БД, а API возвращает идентификатор операции.

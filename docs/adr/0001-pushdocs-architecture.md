...
Уточнение от 10 сентября 2026 года: актуальный [план доработки](../development-plan.md) предусматривает конфигурацию особенностей репозитория без отдельного адаптера sendsay-docs. Полный сайт собирает существующий CI документации после отправки изменений. PushDocs связывает его с текущим SHA через проверку `preview:deploy` и формирует URL по номеру PR/MR. Встроенный runner из [ADR-0002](0002-isolated-draft-preview.md) больше не используется.
...
Согласована структура `Установка → Проекты → Ветки → Документы`. Рабочие области не вводятся: независимые организации в одной установке не требуются, а разные составы пользователей поддерживаются доступом на уровне проекта.
...
Git является источником истины для коммитов, истории и файлов репозитория. PostgreSQL является источником истины для ещё не отправленных черновиков, пользователей, обсуждений CMS и выполняемых операций. Состояния PR/MR, внешних комментариев и CI хранятся в PostgreSQL как обновляемая копия данных провайдера.
...
Для одного контекста ветки допускается один активный общий набор изменений. Разные пользователи могут редактировать разные документы в нём. Для независимой работы над одной страницей пользователь создаёт отдельную ветку. Между проектами одного репозитория задачи отправки сериализуются по репозиторию и ref, а не только по проекту.
...
Вход в CMS не требует персонального аккаунта GitHub/GitLab. Внешние записи выполняются от имени приложения или технического пользователя с явным указанием автора из PushDocs и записью аудита. Пользователь видит, от чьего имени будет опубликован комментарий. Внутренние комментарии не отправляются провайдеру автоматически.
...
Self-Managed подключение содержит API base URL, адрес Git и дополнительный доверенный CA при необходимости. Проверка TLS остаётся включённой. Владелец явно разрешает внутренние адреса GitLab; произвольные приватные адреса, loopback и metadata endpoints недоступны по умолчанию. Правила применяются к DNS, перенаправлениям, API, Git и адресам вложений, а секреты не передаются на другой origin.
...
Установка использует общие локальные учётные записи и приглашения в конкретные проекты. Первый пользователь с системным полномочием владельца создаётся через одноразовый bootstrap-токен, который владелец сервера получает при установке. После инициализации публичное создание первого владельца закрывается. Системное полномочие хранится отдельно от трёх ролей проекта и не является четвёртой ролью команды.

Владелец управляет системными настройками, общими учётными записями и Git-подключениями. Он создаёт проекты, разрешает им конкретные репозитории и назначает первого администратора каждого проекта. Доступ к документам через обычный API требует членства в проекте, в том числе для владельца. Для восстановления управления владелец может явно назначить администратора, включая себя; действие записывается в аудит. Изоляция проектов не является защитой от владельца установки, контролирующего сервер и БД.

В каждом проекте пользователь имеет одну из трёх ролей: администратор, редактор или читатель. Пользователь без членства не видит проект и не получает его данные через API, поиск, вложения или SSE. Отдельные права на документы в первой версии не вводятся: роль действует на все документы и ветки проекта с учётом политик записи в Git. Изменение роли в одном проекте не меняет роли в остальных.

Приглашение содержит проект, адрес получателя, роль и срок действия. При принятии приглашения новый пользователь создаёт учётную запись, а существующий подтверждает свою учётную запись и получает дополнительное членство. Приглашение не меняет пароль существующего пользователя и не выдаёт полномочие владельца. До принятия сервер повторно проверяет действительность приглашения и права пригласившего. Удаление пользователя из проекта сохраняет его учётную запись и доступ к другим проектам.
...
| Управлять настройками и пользователями текущего проекта, политиками веток и архивацией | Да | Нет | Нет |
...
Почтовый сервис необязателен: приглашение можно передать ссылкой вручную, а восстановление доступа владелец выполняет локальной административной командой с аудитом. OIDC/SSO остаётся следующим расширением. Подключение Git-провайдера и вход человека в CMS имеют разные жизненные циклы.
...
| Область | Содержимое |
| --- | --- |
| Левая навигация | Все доступные проекты, переключатель проекта, документы, изменения, PR/MR, файлы, пользователи и настройки текущего проекта; отдельное дерево документов |
...
Переключателя рабочей области нет. Экран пользователей относится к выбранному проекту и показывает роли только в нём. Общие пользователи, Git-подключения и настройки установки находятся в отдельном разделе владельца. Пользователю без проектов показывается отсутствие доступа, а не список чужих проектов. Ранее созданные [концепты экранов](../design/README.md) сохраняются как визуальные ориентиры, но их переключатель Sendsay и общие роли команды устарели.
...
Production Compose содержит `caddy`, `web`, `worker`, `realtime`, `postgres` и одноразовый `migrate`. Миграции выполняются одним процессом до запуска новой версии приложений. Шлюзы OpenVPN входят в отдельный профиль Compose и не запускаются, пока оператор явно не включит VPN для установки. Dev Compose содержит инфраструктуру и воспроизводимые fixtures; GitLab для интеграционных тестов включается отдельным профилем.
...
Установка генерирует секреты и bootstrap-токен, поднимает БД, выполняет миграции и проверяет готовность сервисов. Приложение не требует облачной БД, SaaS realtime, S3 или внешнего identity provider. Для приватного GitLab оператор явно включает профиль VPN установки, а затем добавляет профиль OpenVPN в подключение.
...
Создать Yarn workspaces, Compose, миграции, bootstrap, сессии и проектные роли. Ввести правила зависимостей пакетов по примеру `architecture:check` appnotes. Реализовать аудит, очередь задач и однотипные ошибки API. Критерий завершения: чистая установка, приглашение пользователей с тремя ролями в конкретные проекты, независимые права одного пользователя в разных проектах и восстановление резервной копии.
...
| Удаление последнего администратора проекта или блокировка последнего владельца | Операция отклоняется до назначения замены; восстановление доступа владельцем записывается в аудит |

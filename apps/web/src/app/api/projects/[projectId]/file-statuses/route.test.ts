...
vi.mock("@/lib/provider", () => ({ providerForConnection: mocks.provider }));
...
beforeEach(() => {
  ...
  const snapshot = {
    ...
    store: { listAttachmentsForChangeSet: mocks.attachments },
  };
  mocks.context.mockResolvedValue(snapshot);
  mocks.provider.mockResolvedValue({ compareFiles: mocks.compare });
  mocks.compare.mockResolvedValue([
    { path: "site/docs/a.md", status: "add" },
    { path: "other/a.md", status: "modify" },
  ]);
  mocks.attachments.mockResolvedValue([{ change_set_id: "draft", repository_path: "logo.png" }]);
});
